...
export type SimpleImplementationFn<Context = unknown> = (t: ExecutionContext<Context>) => PromiseLike<void>;
export type SimpleContextFn<Context, T> = (t: ExecutionContext<Context>) => Promise<T>;
...
> /*extends Omit<AvaTestInterface<Context>, 'before' | 'beforeEach' | 'after' | 'afterEach' | 'failing' | 'serial'>*/ {
  ...
  <T extends any[]>(title: string, implementation: Implementation<T, Context>, ...rest: T): void;
  ...
  serial(title: string, implementation: Implementation<unknown[], Context>): void;
  /** Declare a concurrent test that uses one or more macros. Additional arguments are passed to the macro. */
  serial<T extends any[]>(title: string, implementation: Implementation<T, Context>, ...rest: T): void;
  /** Declare a concurrent test that uses one or more macros. The macro is responsible for generating a unique test title. */
  serial<T extends any[]>(implementation: Implementation<T, Context>, ...rest: T): void;

  skip(title: string, implementation: Implementation<unknown[], Context>): void;
  /** Declare a concurrent test that uses one or more macros. Additional arguments are passed to the macro. */
  skip<T extends any[]>(title: string, implementation: Implementation<T, Context>, ...rest: T): void;
  /** Declare a concurrent test that uses one or more macros. The macro is responsible for generating a unique test title. */
  skip<T extends any[]>(implementation: Implementation<T, Context>, ...rest: T): void;
  ...
  context<T extends object | void>(cb: SimpleContextFn<Context, T>): TestInterface<Context & T>;
  contextEach<T extends object | void>(cb: SimpleContextFn<Context, T>): TestInterface<Context & T>;
...
function createTestInterface<Context>(opts: {
  ...
  function assertOrderingForDeclaringSkipUnless() {
    if (suiteOrTestDeclared) {
      throw new Error('skipUnless or if must be declared before declaring sub-suites or tests');
    }
  }
    ...
    const wrapped = async function (t: ExecutionContext<Context>, ...args: any[]) {
      return concurrencyLimiter(
        errorPostprocessor(async () => {
          ...
          return isMacro(impl) ? impl.exec(t, ...args) : (impl as ImplementationFn<any[], Context>)(t, ...args);
        })
      );
    };
    const computedTitle = computeTitle(title, impl, ...args);
    (automaticallySkip || skip ? avaDeclareFunction.skip : avaDeclareFunction)(computedTitle, wrapped, ...args);
  }
  ...
  function test(...inputArgs: any[]) {
    ...
    return declareTest(title, impl, automaticallyDoSerial ? avaTest.serial : avaTest, args);
  }
  ...
  function macro<Args extends any[]>(
    cb: (...args: Args) => [SimpleTitleFn | string, SimpleImplementationFn<Context>] | SimpleImplementationFn<Context>
  ) {
    function title(givenTitle: string | undefined, ...args: Args) {
      const ret = cb(...args);
      return Array.isArray(ret) ? (typeof ret[0] === 'string' ? ret[0] : ret[0](givenTitle)) : givenTitle ?? 'UNKNOWN';
    }
