...
const teardownConcurrencyLimiter = throat(Infinity);
...
function createTestInterface<Context>(opts: {
  ...
  const teardownAllFunctions: Array<() => void> = [];
  function registerTeardownAllFunction(fn: () => void) {
    if(teardownAllFunctions.length === 0) {
      // THIS IS WRONG because the teardown test case is skippable, and it can't be skippable!
      // Each test must declare a teardown() fn that understands if it is the last one.
      declareTest('teardown', [async () => {
        for(const fn of teardownAllFunctions) {
          await fn();
        }
      }], automaticallyDoSerial ? avaTest.serial : avaTest, []);
      teardownAllFunctions.push(fn);
    }
  }
  ...
  test.contextAll = function (
    cb: (test: ExecutionContext<Context>) => Promise<any>
  ) {
    // TODO wrap `test` in something that understands `teardown` should happen only once *all* in-flight tests are finished.
    // Declare teardown as a separate test case and make sure it waits to finish last?
    // And make sure it does not count against concurrency limit?
    
    // Track every time a test starts so we can avoid teardown until all have finished.
    const inflightTests: Promise<void>[] = [];
    assertOrderingForDeclaringHook();
    const cbOnce = once(async (t: ExecutionContext<Context>) => {
      // wrap context so that .teardown callbacks are registered onto our one-time-only
      // teardown testcase
      const teardown: TeardownFn = (fn) => {
        registerTeardownAllFunction(async () => {
          await 
          await fn();
        });
      }
      const tWrapped = Object.assign({}, t, {teardown});
      return await cb(tWrapped);
    });
    beforeEachFunctions.push(async (t: ExecutionContext<Context>) => {
      let res;
      inflightTests.push(new Promise(r => res = r));
      const addedContextFields = await cbOnce(t);
      Object.assign(t.context, addedContextFields);
      t.teardown(res);
    });
    return test;
  };
  test.contextEach = function (
    cb: (test: ExecutionContext<Context>) => Promise<any>
  ) {
    assertOrderingForDeclaringHook();
    beforeEachFunctions.push(async (t: ExecutionContext<Context>) => {
